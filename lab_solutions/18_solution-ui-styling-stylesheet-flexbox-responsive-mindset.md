# Lab 18 – Soluzione (Italian Meals App)

## Cosa mostra la soluzione

- `spacing` tokens in `theme/colors.ts`.
- `createSharedStyles(theme)` centralizzato in `theme/styles.ts`.
- `MealsListScreen` responsive: 1 colonna mobile, 2 colonne tablet (`width >= 600`).
- Layout responsive con `FlatList` a 1 o 2 colonne.
...
> **Nota:** al lab 18 puoi usare un tema fisso (`lightTheme`) se `ThemeContext` arriva al lab 19; l'importante è la struttura `createSharedStyles`.
